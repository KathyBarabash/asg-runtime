    ...
    def _initialize(self, log_level: str = None):
        ...
        for logger_level_pair in log_level.split(","):
            ...
            if logger_name not in logger_names and logger_name != "_default":
                ...
                logging.error("Running without setting unknown logger level.")
            if level not in logging._nameToLevel:
                ...
                logging.error(
                    "Running without setting the level for logger %s.",
                    logger_name,
                )
        ...
        for logger_attr in [attr for attr in dir(Logging) if attr[0] != "_"]:
            ...
            # Check if there is an environmetn override for any of the log files
...
class Formatter(logging.Formatter):
    ...
    def __init__(self):
        # Log message format
        msg_fmt = f"{TEXT['BOLD']}%(name)s:%(levelname)s:{
            TEXT['BOLD_OFF']}%(message)s"
        ...
        self.formatters: dict[int, logging.Formatter] = {
            ...
            logging.WARNING: logging.Formatter(TEXT["YELLOW"] + msg_fmt + TEXT["DEFAULT"]),
